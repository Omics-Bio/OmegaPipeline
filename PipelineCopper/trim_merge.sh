...
##--------------------------
if [ ${ErrorCorrection} == "YES" ]; then
	${PathSga} preprocess --pe-mode 2 \
-o ${WorkingDirectory}/preprocess.fq \
${WorkingDirectory}/filter.fq \
1>${WorkingDirectory}/preprocess.log 2>&1
rm ${WorkingDirectory}/filter.fq

	${PathSga} index -a ropebwt -t ${NumThreads} \
--no-reverse \
${WorkingDirectory}/preprocess.fq \
1>${WorkingDirectory}/index.log 2>&1

	${PathSga} correct -k 41 --learn -t ${NumThreads} \
-o ${WorkingDirectory}/filter.fq \
${WorkingDirectory}/preprocess.fq \
1>${WorkingDirectory}/correct.log 2>&1
fi
